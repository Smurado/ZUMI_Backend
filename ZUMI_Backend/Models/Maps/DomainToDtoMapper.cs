using ZUMI_Backend.Models.Enums.Extensions;
...
public static class DomainToDtoMapper
{
    ...
    => sproject == null ? null : new ProjectDto
        {
            Id = project.Id,
            ...
            Personen = project.Personen?.Select(pp => new PersonRoleDto
            {
                ...
                IsParticipating = pp.ProjectRoleId != null,
            ...
            }).Where(pr => !string.IsNullOrEmpty(pr.FirstName) 
                       || !string.IsNullOrEmpty(pr.LastName)).ToList() 
                       ?? new List<PersonRoleDto>(),
...
// Für Listen (z. B. in ProjectDto)
    ...
    /*spublic static void ApplyCreateFromDto(this Project project, CreateProjectDto dto)
    {
        if (dto == null) return;
    ...
    // --- 1. Mapper für das einzelne Item ---
    public static FundingItemDto MapToFundingItemDto(this FundingItem item)
    {
    ...
    // --- 2. Mapper für die ganze Liste ---
    public static List<FundingItemDto> MapToFundingItemDtos(this IEnumerable<FundingItem> items)
    {
        if (items == null) return new List<FundingItemDto>();
        
        // Nutzt den Einzel-Mapper für jedes Element
        return items.Select(i => i.MapToFundingItemDto()).ToList();
    }
